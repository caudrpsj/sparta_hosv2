...
from wordcloud import WordCloud, STOPWORDS # 워드 클라우드 임포트
...
# client = MongoClient('mongodb://test:test@3.34.188.167',27017)
...
def saving_match(search_receive,number_receive):
  ...
  request_url = 'https://pubmed.ncbi.nlm.nih.gov/?term='+search_receive+'&size=200'
...
def downloadeach():
  ...
  try : 
    (directory, filename) = downloadeach_doi(downloadtarget)
    
  except:   
    print('pmc 확인하러 간다') 
    downloadeach_pmc_yes(downloadtarget) 

  # return jsonify({'result': 'success', 'msg': msg})
  return send_from_directory(directory=directory, filename=filename, as_attachment=True, mimetype='application/pdf', attachment_filename=(str(filename) + '.pdf'))
  # , as_attachment=True, mimetype='application/pdf', attachment_filename=(str(filename) + '.pdf')
...
def downloadeach_doi(downloadtarget):
  ...
  if  downloadtarget_doi.count('[') >1 :
    print('일초과로들어옴 다시 분기')
    if downloadtarget_doi.split('[doi')[0].count('[') > 0 :
      print('DOI가뒤에')
      url = downloadtarget_doi.split('[doi')[0].split('] ')[1]
    else :
      print('DOI가앞에')
  ...
  # outpath = "/home/ubuntu/6.21/static/" + path_name + "/"
...
def downloadeach_pmc(downloadtarget) :
  ...
  driver.find_element_by_xpath('/html/body/div[1]/div[1]/div[2]/div/div[2]/div[1]/div[2]/div[2]/div/ul/li[4]/a').click()
...
@app.route('/makecloud', methods=['GET'])
def makingcloud():
    # 1. 모든 document 찾기 & _id 값은 출력에서 제외하기
    # 2. articles라는 키 값으로 영화정보 내려주기
  articles = list(db.pubmed.find({},{'_id':0}))
  ...
  # stopwords.update(['OM','treatment','We','onychomatricoma','nail','the','tumor','palte','we', 'of','plate','diagnosis','case','the nail','tumor','case','of onychoamtricoma','No abstract available','NO','abstract','report','rare','clinical','review','OM','Conclusion','lichen planus'])
  stopwords.update(['abstract','LP','lesions','clinical','diagnosis','involvement','skin','patients','lichen planus','lichen','planus','lesion','case','disease','Lichen planus','Lichen','No abstract','treatment','No'])
  wc = WordCloud(normalize_plurals=False,collocations=False,stopwords=stopwords, width=1000, height=600, background_color="white", random_state=0, font_path=r'c:\Windows\Fonts\malgun.ttf').generate(abstractcloud_word)
